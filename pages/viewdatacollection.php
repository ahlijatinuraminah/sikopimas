...
<div>
    ...
    <div id="result" style="width: 100%;" class="table-responsive">
        <table id="tblDataCollection" class="table table-bordered table-hover table-striped">
            <thead>
                <tr>
...
<script>
    ...
    function save() {
        if ($("form")[0].checkValidity()) {
            ...
            $.ajax({
                url: "controller/controllerdatacollection.php",
                method: "POST",
                data: objData,
                dataType:"json",  
    ...
    function initializeTable() {
        table = $("#tblDataCollection").DataTable({
            serverSide: false,
            processing: false,
            ajax: {
                url: "controller/controllerdatacollection.php",
    ...
    $(document).ready(function () {
        ...
        $("#modal_button").click(function () {
            resetForm();
            $("#modal").modal("show");
            $(".modal-title").text("Tambah Data Collection");
        });

        $("#tblDataCollection").on("click", ".update", function (e) {
            var data = table.row($(this).closest("tr")).data();
            bindForm(data);
            $("#modal").modal("show");
            $(".modal-title").text("Update Data Collection");           
        });

        $("#tblDataCollection").on("click", ".delete", function (e) {
            var data = table.row($(this).closest("tr")).data();
            var id = data.id;
            swal({
