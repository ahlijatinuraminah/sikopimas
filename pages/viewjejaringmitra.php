...
?>
<div>
    <h2>Master Data Jejaring Mitra Lokal</h2>
    ...
    <div id="result" style="width: 100%;" class="table-responsive">
        <table id="tblJejaringMitra" class="table table-bordered table-hover table-striped">
            <thead>
                <tr>
                    <th style="vertical-align: middle;">No.</th>
                    <th style="vertical-align: middle;">Id.</th>
                    <th style="vertical-align: middle;">Nama Jejaring Mitra Lokal</th>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="myform">
                    <label>Nama Jejaring Mitra Lokal</label>
                    <input type="text" name="nama_mitralokal" id="nama_mitralokal" class="form-control" required />
                    <br />
...
<script>
    ...
    function bindForm(data) {
        $("#id").val(data.id);
        $("#nama_mitralokal").val(data.nama_mitralokal);
    }
    ...
    function save() {
        if ($("form")[0].checkValidity()) {
            ...
            $.ajax({
                url: "controller/controllermitralokal.php",
                method: "POST",
                data: objData,
                dataType:"json",  
    ...
    function initializeTable() {
        table = $("#tblJejaringMitra").DataTable({
            serverSide: false,
            processing: false,
            ajax: {
                url: "controller/controllermitralokal.php",
    ...
    $(document).ready(function () {
        ...
        $("#modal_button").click(function () {
            resetForm();
            $("#modal").modal("show");
            $(".modal-title").text("Tambah Jejaring Mitra Lokal");
        });

        $("#tblJejaringMitra").on("click", ".update", function (e) {
            var data = table.row($(this).closest("tr")).data();
            var id = data.id;
                    bindForm(data);
                    $("#modal").modal("show");
                    $(".modal-title").text("Update Jejaring Mitra Lokal");
            
        });

        $("#tblJejaringMitra").on("click", ".delete", function (e) {
            var data = table.row($(this).closest("tr")).data();
            var id = data.id;
            swal({
