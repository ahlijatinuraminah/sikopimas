   ...
   <div id="diverror" class="col-md-12"></div> 
    ...
    <div class="form-group">
		<label>Nama Organisasi</label>
			<input type="text" name="nama_organisasi" id="nama_organisasi" class="form-control" maxlength="100" required />
		</div>
        <div class="form-group">
		<label>Kepala Perwakilan</label>
			<input type="text" name="representasi" id="representasi" class="form-control" required maxlength="100" />
		</div>
        <div class="form-group">
        ...
        <div class="form-group">
        <label>Alamat</label>
			<textarea rows="4" name="alamat" id="alamat" class="form-control" required maxlength="200"></textarea>
	    </div>
    ...
    <?php
       include "addmasterorganisasi.php";
       //include "viewmitralokalorganisasi.php";
       ?>
   </div>
...
function bindForm(data){	
    ...
    if(data.mitralokalorganisasi != null){
            dataMitraLokal = data.mitralokalorganisasi;
            tableMitraLokal.clear().draw();        
            tableMitraLokal.rows.add(dataMitraLokal).draw();
    }

        if(data.lokasiprovinsiorganisasi != null){
            dataLokasi = data.lokasiprovinsiorganisasi;
            tableLokasi.clear().draw();        
            tableLokasi.rows.add(dataLokasi).draw();
        }

        if(data.isuorganisasi != null){
            dataIsu = data.isuorganisasi;
            tableIsu.clear().draw();
            tableIsu.rows.add(dataIsu).draw();
        }
            
        if(data.bidangkerjaorganisasi != null){
            dataBidangKerja = data.bidangkerjaorganisasi;
            tableBidangKerja.clear().draw();        
            tableBidangKerja.rows.add(dataBidangKerja).draw();
        }

        if(data.donororganisasi != null){
            dataDonor = data.donororganisasi;
            tableDonor.clear().draw();
            tableDonor.rows.add(dataDonor).draw();
        }
        if(data.lokasikabupatenorganisasi != null){        
            dataKabupaten = data.lokasikabupatenorganisasi;
        }
}
function cancel(){
    resetForm();
   ...
   // }
        ...
        $.ajax({
            ...
            data: {
                ...
                id: $('#id').val(), 				   
                mitralokalorganisasi: JSON.stringify(dataMitraLokal),
                lokasiprovinsiorganisasi: JSON.stringify(dataLokasi),
                bidangkerjaorganisasi: JSON.stringify(dataBidangKerja),
